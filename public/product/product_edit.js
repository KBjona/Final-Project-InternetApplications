...
function get_store_id(){ //extracting the store id from the url
    ...
    const url_oarts = page_url.split('/');
    store_id = url_parts.at(-1);
}

function validate_owner(){ //to check if the user is the owner of the store
    get_store_id();
...
form.addEventListener('submit', function (event) {
    ...
    const data = FormData(form); // to get the data from the form in a form data method
    data = Object.fromEntries(data); // converts the data to a key value form 
    ...
    const response = await fetch("api/product/update",{ //sends a fetch request to the server to update the parameters of the store in the database
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(store_id,parameters)
    });
